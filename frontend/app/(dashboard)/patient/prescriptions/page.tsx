  ...
  useEffect(() => {
    const fetchPrescriptions = async () => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Falha ao carregar prescrições",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleDownloadPDF = async (prescriptionId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setError("Falha ao baixar PDF da prescrição");
      }
    } catch (err) {
      console.error("Failed to download PDF:", err);
      setError("Erro ao baixar PDF da prescrição");
    }
  };
        ...
        {!loading && !error && prescriptions.length === 0 && (
          <div className="rounded-lg border border-border bg-card p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1">
              Nenhuma prescrição ainda
            </h3>
            <p className="text-muted-foreground">
              Suas prescrições aparecerão aqui assim que forem emitidas pelo seu médico.
            </p>
          </div>
        )}
                      ...
                      {rx.medicines.map((med, idx) => (
                        ...
                        >
                          ...
                          <div className="text-xs text-muted-foreground mt-1 space-y-0.5">
                            ...
                            {med.instructions && (
                              <p>Instruções: {med.instructions}</p>
                            )}
                          </div>
                        </div>
                      ))}
