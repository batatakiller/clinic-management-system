  ...
  try {
    ...
    if (!patientId || !doctorId || !date || !timeSlot || !reason) {
      return errorResponse(
        res,
        "patientId, doctorId, date, timeSlot, and reason are required.",
        400,
      );
    }
    ...
    if (typeof reason !== "string" || reason.trim().length === 0) {
      return errorResponse(
        res,
        "Reason for visit cannot be empty or contain only whitespace.",
        400,
      );
    }
    ...
    if (!doctor)
      return errorResponse(res, "Doctor not found or inactive.", 404);
