...
exports.getDoctorSchedule = async (req, res, next) => {
    try {
        ...
        return successResponse(res, schedules, 'Doctor schedule retrieved successfully');
    } catch (error) {
        next(error);
    }
};
...
exports.createOrUpdateSchedule = async (req, res, next) => {
    try {
        const { doctorId } = req.params;
        const { dayOfWeek, startTime, endTime, slotDuration, breakTimes } = req.body;

        // Validation
        if (!dayOfWeek || typeof dayOfWeek !== 'number' || dayOfWeek < 0 || dayOfWeek > 6) {
